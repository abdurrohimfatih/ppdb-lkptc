                    ...
                    <div class="card">
                        <div class="row">
                            <div class="col-md-4 pe-md-0">
                                <div class="auth-side-wrapper" style="background-image: url({{asset('img/bg-login1.jpeg')}})"></div>
                            </div>
                            <div class="col-md-8 ps-md-0">
                                <div class="auth-form-wrapper px-4 py-5">
                                    <a href="{{route('login')}}" class="noble-ui-logo d-block mb-2">{{config('app.name')}}</a>
                                    <h5 class="text-muted fw-normal mb-4">Untuk Melakukan Pendaftaran! Silahkan Login atau Daftar Jika Belum Punya Akun</h5>
                                    <form action="{{route('processLogin')}}" class="forms-group" method="post">
                                        {{ csrf_field() }}
                                        @if(session('error') == 1)
                                        <div class="alert alert-danger">
                                            Ouuuppssss... username atau passwordsalah , silahkan diulangi lagi!
                                        </div>
                                        @endif
                                        <div class="mb-3">
                                        ...
                                        <div class="alert alert-primary" >
                                            <a href="{{route('register')}}"  class="alert-link">Jika Belum Punya Akun, Silahkan <b> Klik Disini </b></a>
                                        </div>
                                    </form>
                                </div>
                            </div>
